...
CREATE  PROCEDURE [Medical].[usp_getDataTypeErrors]        
(        
...
BEGIN TRY        
 ...
 BEGIN TRAN        
  ...
  INNER JOIN sys.types u         
   ...
   ON C.Name=D.name    
   AND D.object_id=(SELECT OBJECT_ID (CONCAT(@schemaName,'.',@ffTable) ) )    
  ORDER BY c.name ASC;         
  ...
  WHILE (@ctr>0)         
   BEGIN          
    SELECT @q=          
     'UPDATE ['+ @schemaName+ '].[' + @ffTable +'] '+   'SET GoodToImport = 0, '+   'ErrorDescription = CONCAT(ErrorDescription,''| Error Column Name is ['+colname +        '] '' + ''and value is : (''+ CAST(['+colname+'] as nvarchar(max))+'')'') '+  'WHE
RE [' + colName + '] IS NOT NULL AND TRY_CAST(['+colname+'] as '+dataType+') IS NULL; '  FROM @temp WHERE rownum=@ctr;         
        
   SELECT @q= @q+         
     'UPDATE ['+ @schemaName+ '].[' + @ffTable +'] '+   'SET GoodToImport = 0, '+   'ErrorDescription = CONCAT(ErrorDescription,''|Truncation Error Column Name is ['+colname +        '] '' + ''and value is : (''+ CAST(['+colname+'] as nvarchar(max))+'')''
) '+  'WHERE CAST(LEN([' + colName + ']) AS NVARCHAR(MAX)) > ' + CAST( dataTypeLength AS NVARCHAR(MAX)) + ' ;' FROM @temp WHERE rownum=@ctr AND datatypeLength IS NOT NULL;         
  PRINT @q;      
    EXEC(@q);        
